...
import { SpannerDriver, SpannerFailureText, SpannerOperationError } from '@proteinjs/db-driver-spanner';
 ...
 * values and never the backend's own message (SpannerFailureText; FailureText.test.ts).
 */
...
describe('SpannerOperationError.statementShape — the verb and table, never a value', () => {
  ...
  test('summarize names the gRPC status and the recognized class of a coded error, and withholds the message of anything else', () => {
    expect(
      SpannerFailureText.summarize(Object.assign(new Error('Row [k] in table t already exists'), { code: 6 }))
    ).toEqual({
      code: 6,
      status: 'ALREADY_EXISTS',
      failureClass: 'row already exists',
      message: 'a row with this primary key already exists',
    });
    expect(SpannerFailureText.summarize(new Error('deadline')).failureClass).toBe('unclassified');
    expect(SpannerFailureText.summarize(new Error('deadline')).message).not.toContain('deadline');
    expect(SpannerFailureText.summarize('boom').message).not.toContain('boom');
  });
});
...
describe('Data-op failures carry their cause (emulator)', () => {
  ...
  test('a duplicate-key insert throws the typed error: status + statement + the failure`s class in the message, the vendor error behind vendorError, its code kept', async () => {
    ...
    expect(failure.message).toBe(
      'Failed when executing dml (ALREADY_EXISTS, code 6) on INSERT db_test_operation_failure_cause: a row with this primary key already exists'
    );

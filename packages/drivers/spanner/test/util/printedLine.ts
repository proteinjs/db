 ...
 * `console.*`) and with hidden properties shown and getters run; its serialized form (a structured
 * writer); the standard `cause` chain an error reporter walks, message and `details` at each link;
 * and every own property of the error, enumerable or not.
 */

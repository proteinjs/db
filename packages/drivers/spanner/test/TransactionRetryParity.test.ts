 ...
 * library's transaction runner to EXACTLY the retries it was led to while the backend's message
 * still rode the driver's typed error — never fewer, never more.
 *
 ...
 *
 * Every scenario goes through the driver's REAL failure path at one of the four doors a
 * transaction body's error comes through (a statement on a caller's transaction: `runDml`,
 * `runQuery`; the driver's own single-statement transaction; the commit), with the vendor client's
 * rejection stubbed in place of the RPC. EXPECTED is not computed by this file: it is the verdict
 * RECORDED from the released driver (TransactionRetryParity.recorded.json — this same file, run
 * once with `RETRY_PARITY_RECORD=<path>` on a checkout of the release line, where the typed error
 * still carried the backend's masked message and a failed commit threw the vendor's own error).
 *
 * The matrix: every retry signal x bound values that overlap its literal (the round where striking
 * bound values lost retries), and the backend ECHOING a bound literal in each form it prints a
 * value — quoted, braced, bracketed (masked on the release line: a final failure), bare,
 * parenthesized, split by whitespace, past the bounded line (the round where reading the raw
 * message added retries without end).
 */

type Door = 'dml on a transaction' | 'query on a transaction' | 'single-statement dml' | 'commit';
...
const DOORS: Door[] = ['dml on a transaction', 'query on a transaction', 'single-statement dml', 'commit'];
...
const statusName = (code: number) => (code === 5 ? 'NOT_FOUND' : code === 13 ? 'INTERNAL' : `CODE_${code}`);
...
const scenarios = (): Scenario[] => {
  ...
  // the client library itself cannot read such an error at the commit door).
  for (const door of DOORS.filter((each) => each !== 'commit')) {
...
describe('the retry law: the runner`s verdict on the thrown error, scenario by scenario, against the recorded release line', () => {
  ...
  test('the recording holds BOTH directions (it is not all one verdict), at every door', () => {
    ...
    for (const door of DOORS) {
      ...
      // The direction that ran away: a quoted/braced/bracketed echo is final at a statement's
      // door, where the runner was shown the masked line; a failed commit reached it unmasked.
      expect(at(`echo: braced+quoted key | ${SESSION}`)).toBe(
        door === 'commit' ? 're-run on a fresh session' : 'final'
      );
      expect(at('echo: double-quoted | RST_STREAM')).toBe(
        door === 'commit' ? 'the runner re-runs it after 1ms' : 'final'
      );
    }
  });
});

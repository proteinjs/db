 ...
 * behind its `vendorError()` method instead — never as `cause`, never as a property of the
 * instance, never as an accessor — so the vendor's raw message, which quotes the offending row key, never rides a line

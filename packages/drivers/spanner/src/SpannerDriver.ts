...
export class SpannerDriver implements DbDriver {
   ...
   * every line the driver writes about a statement, at every level: each parameter's NAME, its
   * TYPE and, for strings, arrays and bytes, its LENGTH. Never a value: a parameter is row content
   * (a presented token, a credential hash, an address), and a log line outlives and out-travels the
   * row it came from. The SQL text beside it carries placeholders only, so name + type + length is
   * what locates a failure (which parameter was null, which was oversized) without quoting it.
   *

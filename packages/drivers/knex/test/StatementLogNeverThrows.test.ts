...
describe('KnexFailureLine.causeOf is total: anything thrown, a summary of codes and the driver`s sentence, never the vendor`s text', () => {
  ...
  const THROWN: [string, () => unknown, { [fact: string]: unknown }][] = [
    ...
    ['nothing', () => undefined, { message: UNNAMED }],
    ['null', () => null, { message: UNNAMED }],
    ['a thrown string', () => CANARY, { message: UNNAMED }],
    ['a thrown number', () => 1146, { message: UNNAMED }],
    [
    ...
    [
      ...
      { name: 'Error', errno: 1146, sqlState: '42S02', message: UNNAMED },
    ],
    ['an error that refuses every read', () => refusesEverything(new Error(CANARY)), { message: UNNAMED }],
    ['a revoked error', () => revoked(new Error(CANARY)), { message: UNNAMED }],
  ];

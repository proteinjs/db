  ...
  onRequest: async (request, response): Promise<void> => {
    ...
    try {
      ...
      if (ServiceRefusal.is(error)) {
        // A deliberate refusal, not a failure — a file no copy can be made of is not there for
        // anyone but its owner (`FileStorage` answers every door the same 404 and keeps the maker's
        // reason in its own WARN). The same words as a row the caller cannot read; quiet, never a 500.
